 ...
 A: • 可以，不用改算法，在导出/发送调度数据时做预处理：
  ...
  那么确定的只要是工作日就8-12 14-18的工作时间是否也不用处理，只要在调用接口时固定？


• 是的，如果业务确定“所有工作日统一 8-12、14-18”且不区分产线/班次，最简单的做法是在调用/导出调度数据时直接按固定时段写
  入，不必存库或再计算。配合当前日历接口：

  - 用 /api/calendar/check 或一次性拉区间事件，先判断某天是否工作日；
  - 对工作日，直接使用固定时间窗：上午 8-12、下午 14-18；无需额外存储。
  - 如未来出现特殊班次/弹性时段，再扩展接口返回具体时间段。
